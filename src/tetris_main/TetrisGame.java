...
public class TetrisGame extends JPanel { // Inherits from JPanel in order to make the GUI
	...
	public void newPiece() {
		...
		rotation = 0;//initially there is  always no rotation
	...
	@Override 
	public void paint(Graphics gui)//Also includes game over
	{
		...
		if (!collidesAt(pieceOrigin.x, pieceOrigin.y, rotation))//if the new tetromino collides to the an older tetromino and it is still able to move
		{
	...
	}else
	{
        ...
        jframe.setBounds(500, 300, 400, 100);//Creates the new boundaries of the timer box

        jframe.add(jLabel);
        jframe.setVisible(true);//shows the box when game over

        Timer timer = new Timer(); // creates a new instance of timer

        timer.scheduleAtFixedRate(new TimerTask() {//the fixed rate is the timer following in seconds
            int i = 3;//inital time shown is 3

            public void run() {

                jLabel.setText("Time left untill the Game closes: " + i);//What is show within the box
                i--;

                if (i < 0) {
                    timer.cancel();
                    jLabel.setText("Time Over");
                    System.exit(0);//When the timer reaches 0 the program would end
                }
            }
        }, 0, 1000);//What the fixed rate is (in ticks which is microseconds)
		gui.setFont(new Font("arial",Font.BOLD, 30));//Shows what it the qualities of the text
		gui.drawString("Game Over", 100, 300);
